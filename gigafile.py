...
def upload_file_and_get_link2(file_path:str, password=''):
        options = Options()
        ...
        try:
            service = EdgeService(service_args=['--log-level=OFF'], log_output=None)
...
def upload_file_and_get_link(file_path:str, password=''):
        options = Options()
        options.add_argument("--headless")
        options.add_argument("--log-level=3")
        options.add_experimental_option("excludeSwitches", ["enable-automation"])
        ...
        try:
            ...
            print("再度実行中....")
            time.sleep(1)
            upload_file_and_get_link2(file_path, password)
